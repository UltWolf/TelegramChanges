...
namespace TelegramChanges
{
                    ...
                    if ((message.Text == _messages[_messages.Count - 1].Text) && (message.Text == _messages[_messages.Count - 2].Text))
                    {
                        if ((message.From.Id == _messages[_messages.Count - 1].From.Id) && (message.From.Id == _messages[_messages.Count - 2].From.Id))
                        {
                            try
                            {
                                ...
                                await Bot.SendPhotoAsync(message.Chat, photo: "https://i.ytimg.com/vi/8oKG9KV9NG8/hqdefault.jpg", caption: "Сегодня мы прощаемся с рабом Божьми : " + message.From.FirstName);
                                _blackListMember.Add(await Bot.GetChatMemberAsync(message.Chat, message.From.Id));
                                int banSeconds = 10;
    ...
    parseMode: ParseMode.Html
                    ...
                    case "/unban":
                        ...
                        try
                        {
                            int userId = _blackListMember.Find(m => m.User.Username == message.Text.Split(' ')[1]).User.Id;
                            await Bot.UnbanChatMemberAsync(message.Chat, userId);
                        }
                        catch(Exception ex)
                        {
                            await Bot.SendTextMessageAsync(message.Chat, "К сожалению мы не можем найти, или убрать со списка данного гражданина.");
                        }
                        break;
